...
// 0 = There will be no further authentication. You will have to handle this by yourself!
// 1 = You need to change ADMIN_PASSWORD to make this work!
const USE_AUTHENTICATION = 1; // Use (internal) authentication - best choice if
const ADMIN_USERNAME = 'apc'; // Admin Username
const ADMIN_PASSWORD = 'password'; // Admin Password - CHANGE THIS TO ENABLE!!!
const DATE_FORMAT = 'Y/m/d H:i:s'; // US
const GRAPH_SIZE = 200; // Image size
const PROXY = 'tcp://127.0.0.1:8080';
...
// https://github.com/martinille/PHP-library/blob/main/functions/php_70/file_get_contents_ssl.php
...
// create graphics
function graphics_avail(): bool {
	return extension_loaded('gd');
}

if (isset($MYREQUEST['IMG'])) {
	error_reporting(0); // we will output non-text content so errors could crack it

	if (!graphics_avail()) {
		header("content-type: image/svg+xml");
		echo '
			<svg width="300" height="36" viewBox="0 0 300 36" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
				<g transform="translate(0 0)">
				<rect rx="0" transform="translate(0 0)" fill="white"></rect>
				<text transform="translate(0 23)" fill="#9e9e9e" font-size="15">
				<tspan x="0" y="0">GD extension not loaded.</tspan></text>
				</g>
			</svg>
		';
		exit();
...
function sortheader($key, $name, $extra = ''): string {
...
// create menu entry
function menu_entry($ob, $title): string {
...
function put_login_link($s = "Login") {
	...
	elseif (ADMIN_PASSWORD == 'password') {
		echo '<a href="#" onclick="alert(\'You need to set a password at the top of apc.php before this will work!\');return false">' . $s . '</a>';
	}
	elseif ($AUTHENTICATED) {
		echo $_SERVER['PHP_AUTH_USER'] . "'&nbsp;logged&nbsp;in!";
	}
	else {
		print "<a href=\"$MY_SELF&LO=1&OB={$MYREQUEST['OB']}\">$s</a>";
	}
}

function block_sort(array $array1, array $array2): int {
...
EOB;
		...
		<tr class="tr-0">
			...
			<td>{$mem['num_seg']} Segment(s) with $seg_size <br> ({$cache['memory_type']} memory)</td>
        </tr>
EOB;
		...
		<div class="info div2"><h2>Runtime Settings</h2>
...
EOB;
		$rss = file_get_contents_ssl("https://pecl.php.net/feeds/pkg_apcu.rss");
		if (!$rss) {
